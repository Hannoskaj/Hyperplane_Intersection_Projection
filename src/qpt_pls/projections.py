...
def proj_TP(rho):
    ...
    correction = N.einsum(
        "de, fg -> dfeg",
        partial_mixed,
        (partial_mixed - N.trace(rho.reshape(4 * [d]), axis1=0, axis2=2)),
    )
...
def new_proj_CP_threshold(
    rho, free_trace=True, full_output=False, thres_least_ev=False
):
    """
    ...
    * if thres_least_ev=True, free_trace is ignored. Then we threshold by minus
    ...
    """
    eigvals, eigvecs = SL.eigh(
        rho
    )  # Assumes hermitian; sorted from lambda_min to lambda_max
    ...
    rho_hat_TLS = (eigvecs[:, indices_positifs] * eigvals[indices_positifs]) @ eigvecs[
        :, indices_positifs
    ].T.conj()
    ...
    * if thres_least_ev=True, free_trace is ignored. Then we bound from below all 
    ...
    if thres_least_ev:
        ...
        if missing < 0:  # On this rare event, revert to usual projection
            ...
            eigvals[last_more_missing] = (
                eigvals[last_more_missing] + missing - inv_cum_toadd[last_more_missing]
            )
    else:
        eigvals = eigvals.clip(0)
        if not free_trace:
...
def step(XW, sq_norm_xn):
    ...
    for i in range(nb_active - 2, -1, -1):
        ...
        if test:
            subset = [i] + subset
            coeffs = la(
                XW[N.ix_(subset, subset)], sq_norm_xn
            )  # Always positive ??? Vérifier
...
def HIP_switch(
    ...
    if alt_to_HIP_switch == "cos":
        w_norm_ancien = N.zeros(
            (dim2, dim2)
        )  # Not normalized to ensure at least two steps are taken.
