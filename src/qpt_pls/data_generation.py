...
def Choi_LS_from_Pauli_freq(k, freq, optimize="optimal"):
    ...
    Mk = M_k(k)
    Choi_est = N.einsum(
        "nm, nde, mfg -> fegd", freq, Mk, Mk, optimize=optimize
    )  # Ordre des indices étrange
    return Choi_est.reshape(4 ** k, 4 ** k) / 18 ** k


def Choi_LS_Pauli_from_channel_bigmem(
    k, channel, cycles=1, optimize="optimal", full_output=1
):
    """
...
def Choi_LS_Pauli_from_channel_mem(
    k, channel, cycles=1, optimize="optimal", full_output=1
):
    """
...
def MUBS(p):
    ...
    MUBvecs[1:p] = w ** (gamma * (j + k) ** 2) / N.sqrt(
        p
    )  # I don't know why Ivanovic uses j+k+1
    return MUBvecs
...
def probas_MUBS(p, channel, optimize="optimal"):
    ...
    for kraus in channel:
        a = N.einsum(
            "bvj, ij, cwi -> bvcw", MUBvecs, kraus, MUBvecs.conj(), optimize=optimize
        )
        probas += a.real ** 2 + a.imag ** 2
...
def Choi_LS_MUBS_from_freq(p, freq, optimize="optimal"):
    ...
    Choi1 = N.einsum(
        "bvcw, bvde, cwfg -> fegd", freq, M, (p + 1) * M / p, optimize=optimize
    )
