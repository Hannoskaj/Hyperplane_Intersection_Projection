...
def sub_basis(d, indices_list):
    ...
    return N.array(
        [
            N.linalg.matrix_power(cl, i) @ N.linalg.matrix_power(syl, j)
...
def rand_unitary(dim):
    ...
    z = (
        1
        / N.sqrt(2)
        * (SS.norm.rvs(size=(dim, dim)) + 1j * SS.norm.rvs(size=(dim, dim)))
    )
    q, r = SL.qr(z)
    d = r.diagonal()
    q *= d / N.abs(d)
...
def convex_combi_channels(d, weights, channels):
    ...
    * channels: list of channels
    ...
    assert N.isclose(
        weights.sum(), 1
    ), "Not trace-preserving; \sum w_c[0] must equal 1."
    coeffs = N.sqrt(weights)
    Kraus = N.concatenate(
        [coeff * channel for (coeff, channel) in zip(coeffs, channels)]
    )
    return Kraus
